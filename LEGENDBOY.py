...
lnbot = TelegramClient("royalboy", API_ID, API_HASH).start(bot_token=token)


class RoyalBoy(TelegramClient):
...
if Config.ROYAL_STRING:
    session_name = str(Config.ROYAL_STRING)
    sweetie = RoyalBoy(session_name, Config.APP_ID, Config.API_HASH)
else:
