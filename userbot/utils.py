...
from userbot import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, Royal, RoyalBot, bot
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        mod.royal_cmd = admin_cmd
        mod.sudo_cmd = sudo_cmd
        # support for ROYALBOT originals
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        name = "userbot.plugins.RoyalBot-Addons.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["RoyalBot-Addons." + shortname] = mod
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/Samim3316)"
