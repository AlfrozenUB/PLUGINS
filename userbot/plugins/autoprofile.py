...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "Royalary ROYALBOT"
DEL_TIME_OUT = 60
...
async def _(event):
    if event.fwd_from:
        return
    ROYAL = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    ...
    await ROYAL.edit(f"Auto Name has been started my Master")
