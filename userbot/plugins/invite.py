...
from ROYALBOT.utils import admin_cmd, edit_or_reply, eor, sudo_cmd
...
async def get_users(event):
    ...
    royal_chat = legen_.lower
    restricted = ["@BR_guild", "@The_royal_users"]
    royal = await eor(event, f"**Inviting members from** {legen_}")
    if royal_chat in restricted:
    ...
    if not sender.id == me.id:
        ROYAL = await edit_or_reply(event, "`processing...`")
    else:
        ROYAL = await edit_or_reply(event, "`processing...`")
    royal = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await ROYAL.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await ROYAL.edit(
        "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/BR_guild)**\n\n`🔸Inviting Users.......`"
    )
    async for user in event.client.iter_participants(royal.full_chat.id):
        try:
...
async def get_users(event):
    ...
    await ROYAL.edit("**TerminalStatus**\n\n`Collecting Users.......`")
