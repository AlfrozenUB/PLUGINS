...
class Royal_GlobalCollection(BASE):
    __tablename__ = "royal_globalcollection"
    ...
    def __repr__(self):
        return "<Royal Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Royal_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Royal_GlobalCollection.__table__.create(checkfirst=True)

ROYAL_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with ROYAL_GLOBALCOLLECTION:
        keyword_items = Royal_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with ROYAL_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Royal_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with ROYAL_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Royal_GlobalCollection.keywoard)
            .filter(Royal_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Royal_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Royal_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Royal_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Royal_GlobalCollection).all()
        for x in all_groups:
