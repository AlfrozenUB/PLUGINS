...
class Royal_GlobalCollection_Json(BASE):
    __tablename__ = "royal_globalcollectionjson"
...
Royal_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(Royal_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
