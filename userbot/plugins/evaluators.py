...
async def _(event):
    ...
    ROYALuser = await event.client.get_me()
    if ROYALuser.username:
        curruser = ROYALuser.username
    else:
        curruser = "@The_royal_users"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    if EVAL == "ON":
        ...
        ROYALevent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec",
    "<command>",
    "Execute a Terminal command on ROYALBOT server and shows details",
).add_command(
