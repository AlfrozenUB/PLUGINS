...
async def bigspam(ROYAL):
    if not ROYAL.text[0].isalpha() and ROYAL.text[0] not in ("/", "#", "@", "!"):
        ROYAL_msg = ROYAL.text
        ROYALBOT_count = int(ROYAL_msg[9:13])
        ROYAL_spam = str(ROYAL.text[13:])
        for i in range(1, ROYALBOT_count):
            await ROYAL.respond(ROYAL_spam)
        await ROYAL.delete()
        if LOGGER:
            await ROYAL.client.send_message(
    ...
    if lg_id:
        if event.is_private:
            ...
            await event.client.send_message(
                lg_id,
                "#WSPAM\n"
                + f"Word Spam was executed successfully in {royal_mention} chat with : `{message}`",
            )
