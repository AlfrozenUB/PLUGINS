...
"""
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        if PM_WARNS[chat_id] == Config.MAX_FLOOD_IN_PM:
            r = await event.reply(ROYAL_ZERO)
            await asyncio.sleep(3)
        ...
        royal_ = await tap[0].click(event.chat_id)
        PM_WARNS[chat_id] += 1
        chat_id = chat_id
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
        PREV_REPLY_MESSAGE[chat_id] = royal_
...
@bot.on(events.NewMessage(incoming=True, from_users=(2020686239)))
...
CmdHelp("pm_permit").add_command(
    ...
    "listapproved", None, "Sends the list of all users approved by RoyalBot"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
