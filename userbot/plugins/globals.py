...
async def _(royalevent):
    i = 0
    await royalevent.get_sender()
    me = await royalevent.client.get_me()
    royal = await eor(royalevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await royalevent.get_chat()
    if royalevent.is_private:
        user = royalevent.chat
        rank = royalevent.pattern_match.group(1)
    else:
        royalevent.chat.title
    try:
        user, rank = await get_full_user(royalevent)
    except:
        pass
    if me == user:
        await royal.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "ℓεɠεɳ∂"
    except:
        return await royal.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await royalevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await royalevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await royal.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await royal.edit(f"**Reply to a user !!**")
    await royal.edit(
...
async def _(royalevent):
    ...
    await royalevent.get_sender()
    me = await royalevent.client.get_me()
    royal = await eor(royalevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await royalevent.get_chat()
    if royalevent.is_private:
    ...
    try:
        ...
        pass
    if me == user:
        await royal.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "royal"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await royalevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await royal.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await royal.edit(f"**Reply to a user !!**")
...
async def _(event):
    royal = await eor(event, "`Gbanning...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(
            royal, "**To gban a user i need a userid or reply to his/her message!!**"
        )
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == Samim3316:
        return await eod(royal, "🥴 **Nashe me hai kya lawde ‽**")
    ...
    if a is not None:
        ...
        gbpic = gban_pic
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **Gbanned** By {royal_mention} \n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {royal_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(royal, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eor(royal, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                await event.client.edit_permissions(
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            royal = int(hel)
            try:
                tity = await event.client.get_entity(royal)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={royal}) (`{royal}`)\n"
    else:
...
async def gkick(event):
    royal = await eor(event, "`Kicking globally...`")
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(royal, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == Samim3316:
        return await eod(royal, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eor(royal, "**😪 I'm not going to gkick my developer!!**")
