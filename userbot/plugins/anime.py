...
async def nope(lege):
    royal = lege.pattern_match.group(1)
    if not royal:
        if lege.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(royal))}")
...
async def nope(lege_):
    royal = lege_.pattern_match.group(1)
    if not royal:
        if lege_.is_reply:
            (await lege_.get_reply_message()).message
        else:
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(royal))}")
...
async def nope(lege_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(royal))}")
