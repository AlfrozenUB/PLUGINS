...
async def nope(aura):
    ROYAL = aura.pattern_match.group(1)
    if not ROYAL:
        if aura.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(ROYAL))}")
...
async def _(event):
    ...
    thumb_name = f"thumb{Samim3316}.jpg"
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [ROYALBOT](https://t.me/ROYALSupport)",
            )
            await event.delete()
        except YouBlockedUserError:
