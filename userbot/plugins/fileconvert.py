...
from ROYALBOT.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(ROYAL):
    if ROYAL.fwd_from:
        return
    reply_to_id = ROYAL.message.id
    if ROYAL.reply_to_msg_id:
        reply_to_id = ROYAL.reply_to_msg_id
    event = await edit_or_reply(ROYAL, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await ROYAL.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await ROYAL.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    ROYALreply = await event.get_reply_message()
    if not ROYALreply or not ROYALreply.media or not ROYALreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if ROYALreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    ROYALevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, ROYALreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await ROYALevent.edit("`This file is not supported`")
            ROYALresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            ROYALfile = Path(
                await event.client.download_media(ROYALresponse, "./temp/")
            )
            ROYALgif = Path(await unzip(ROYALfile))
            royal = await event.client.send_file(
                event.chat_id,
                ROYALgif,
            ...
            await event.client(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=royal.media.document.id,
                        access_hash=royal.media.document.access_hash,
                        file_reference=royal.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await ROYALevent.delete()
            for files in (ROYALgif, ROYALfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await ROYALevent.edit("Unblock @tgstogifbot")
            return
