...
ROYAL = str(ALIVE_NAME) if ALIVE_NAME else "Du"
...
async def dc(event):
    await event.edit(f"{ROYAL}: Running Terminal.....")
    ...
    if cmd in ("userbot.session", "env", "printenv"):
        return await event.edit(f"{ROYAL}: Privacy Error, This command not permitted")
    ...
    await event.client.send_file(
        ...
        "term.txt",
        reply_to=event.id,
        caption=f"{ROYAL}: Output too large, sending as file",
    )
    os.remove("term.txt")
    return
